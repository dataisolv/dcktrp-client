...
import { DOCUMENT_DIVISION_OPTIONS, DOCUMENT_ACCESS_OPTIONS } from '@/lib/constants';
...
export default function ChatSettingsPanel({ settings, onSettingsChange }: ChatSettingsProps) {
    ...
    const toggleAccessFilter = (access: string) => {
        const current = settings.access_filter;
        if (current.includes(access)) {
            updateSetting('access_filter', current.filter((a) => a !== access));
        } else {
            updateSetting('access_filter', [...current, access]);
        }
    };
            ...
            <SheetContent className="w-[400px] sm:w-[540px] overflow-y-auto">
                ...
                <div className="space-y-6 py-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {DOCUMENT_ACCESS_OPTIONS.filter(a => a.value !== 'ALL').map((access) => (
                                <div key={access.value} className="flex items-center space-x-2">
                                    <input
                                        type="checkbox"
                                        id={`access-${access.value}`}
                                        checked={settings.access_filter.includes(access.value)}
                                        onChange={() => toggleAccessFilter(access.value)}
                                        className="rounded border-gray-300"
                                    />
                                    <Label
                        ...
                        <p className="text-xs text-gray-500">
                            {settings.access_filter.length === 0
                                ? 'All access levels (no filter)'
                                : `${settings.access_filter.length} selected`}
                        </p>
                    </div>
